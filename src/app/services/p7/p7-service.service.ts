...
export class P7ServiceService {
  ...
  getBooks()
  {
    return this.http.get(this.url+"practica7");
  }
  bookStore(form:Book)
  {
      return this.http.post<any>(this.url+'practica7',form) .pipe(
        retry(1),
  ...
  {
    return this.http.put<any>(this.url+'practica7/'+id,form) .pipe(
      retry(1),
      catchError((error: HttpErrorResponse) => {
        if (error.status === 422) {
          window.alert('Fill all the fields');
  ...
  deleteBook(id:any)
  {
    return this.http.delete<any>(this.url+'practica7/'+id) .pipe(
      retry(1),
      catchError((error: HttpErrorResponse) => {
        if (error.status === 422) {
          window.alert('Fill all the fields');
