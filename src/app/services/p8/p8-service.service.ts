...
export class P8ServiceService {
  ...
  getBooks()
  {
    return this.http.get(this.url+"practica8");
  }
  getBook(id:any)
  {
    return this.http.get(this.url+"practica8B/"+id);
  }

  bookStore(form:Book)
  {
      return this.http.post<any>(this.url+'practica8',form) .pipe(
        retry(1),
  ...
  {
    return this.http.put<any>(this.url+'practica8/'+id,form) .pipe(
      retry(1),
      catchError((error: HttpErrorResponse) => {
        if (error.status === 422) {
          window.alert('Fill all the fields');
  ...
  deleteBook(id:any)
  {
    return this.http.delete<any>(this.url+'practica8/'+id) .pipe(
      retry(1),
      catchError((error: HttpErrorResponse) => {
        if (error.status === 422) {
          window.alert('Fill all the fields');
